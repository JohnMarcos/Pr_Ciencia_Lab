...
Enrollment.create!(user_id: john.id, course_id: Course.all.sample.id)
Enrollment.create!(user_id: john.id, course_id: Course.all.sample.id)
lesoon_id = Enrollment.all.select { |enrollment| enrollment.user_id == john.id }.each do |enrollment|
  enrollment.course.lessons.id.sample
Comment.create!(user_id: john.id, lesson_id: lesson_id, comment: 'Muy bueno el curso')
end
